...
const TABS = [
  ...
  { href: '/briefing', icon: Sunrise, label: '오늘의 브리핑', short: '브리핑' },
        ...
        <aside className="w-64 border-r bg-card flex flex-col">
          <div className="p-4 border-b flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-xl">InfoHub</span>
            </div>
            <div className="text-right text-xs text-muted-foreground">
              <div className="font-mono">{formatTime(currentTime)}</div>
              <div>{currentTime.toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' })}</div>
            </div>
          </div>
        ...
        <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
          ...
          <SheetContent side="left" className="w-72 p-4">
            ...
            <div className="text-right text-xs text-muted-foreground mb-4">
              <div className="font-mono" id="mobile-clock"></div>
            </div>
...
function ThemeSelector({ theme, setTheme }: { theme: string; setTheme: (t: 'light' | 'dark' | 'amoled' | 'system') => void }) {
  const themes = [
...
function ViewModeToggle({ viewMode, setViewMode }: { viewMode: string; setViewMode: (t: 'mobile' | 'desktop') => void }) {
  return (
    <Button
      variant="outline"
